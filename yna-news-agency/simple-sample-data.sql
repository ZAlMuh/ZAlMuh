-- Simple Sample Data for YNA - Youth News Agency
-- IMPORTANT: First create a user through Supabase Auth Dashboard, then run this

-- Step 1: Go to Supabase Dashboard → Authentication → Users → Create new user
-- Use email: admin@yna.news and password: password123
-- Note down the user ID from the dashboard

-- Step 2: Replace 'YOUR_USER_ID_HERE' below with the actual user ID from Step 1
-- Then run this script in Supabase SQL Editor

-- Insert user profile (replace the UUID with your actual user ID)
INSERT INTO public.users (
  ...
  'YOUR_USER_ID_HERE',  -- Replace this with the user ID from Supabase Auth
  'YNA Admin',
  'admin@yna.news',
  'admin'
...
ترقبوا محتوى مثيراً وأخباراً عاجلة وتحليلات متعمقة وقصصاً تلهم وتعلم. مرحباً بكم في مجتمع وكالة أنباء الشباب!',
  ...
  'published',
  true,
  'YOUR_USER_ID_HERE'  -- Replace this with the same user ID
),
(
